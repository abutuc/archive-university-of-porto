...
export function FileCard({
  ...
  if (viewMode === "grid") {
    return (
      <div
        className="bg-[#f6f9ff] border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition cursor-pointer flex flex-col h-[220px] min-w-0"
        onClick={onSelect}
      ...
      >
        <div className="flex-1 flex items-center justify-center">
          <div className="text-[28px]">{getFileIcon()}</div>
        </div>
        ...
        <div className="w-full border-t border-gray-100 bg-white rounded-b-2xl flex items-center justify-between px-3 py-2 min-h-[44px]">
          <span className="font-medium text-gray-700 truncate max-w-[140px]">{file.name}</span>
          <DropdownMenu>
