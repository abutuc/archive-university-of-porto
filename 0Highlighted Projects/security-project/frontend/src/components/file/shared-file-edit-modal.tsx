...
import { FileEditModalProps } from "../types/props";
...
export function FileEditModal({
  ...
  const handleSave = async () => {
    ...
    if (newName !== file.name) {
      const res = await fetch(`${API_BASE_URL}/files/shared/${file.id}/name`, {
        method: "PUT",
        headers: {
    ...
    if (newFile) {
      ...
      const res = await fetch(`${API_BASE_URL}/files/shared/${file.id}`, {
        method: "PUT",
        credentials: 'include',
        body: formData,
      });
